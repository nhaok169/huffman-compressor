...
### 编码表示例
假设字符'A'的编码为`101`（3位），则存储为：
- 字符：`0x41` ('A'的ASCII)
- 长度：`0x03` (3位)
- 编码数据：`0xA0` (二进制`10100000`，高位对齐)
...
### Q2: 如何确认压缩文件的完整性？
**A:** 压缩文件前4字节应为"HUFF"（十六进制`48 55 46 46`），可以使用hex编辑器验证。

### Q3: 查找功能为什么有时会找到错误的位置？
**A:** 由于哈夫曼编码是变长的，短字符串的编码可能在比特流中偶然匹配。建议：
1. 使用较长的搜索词（≥3字符）
2. 验证找到的上下文是否合理
3. 程序已包含边界检查，但仍存在理论上的伪匹配可能
...
### 对于大文件（>10MB）
1. 考虑分块压缩（当前版本不支持，需修改）
2. 调整查找窗口大小（修改finder.h中的MAXR定义）
...
### 开发建议
1. **修改字符范围**：在encoder.cpp中调整字符检查逻辑
2. **优化内存**：调整finder.cpp中的MAXR值
3. **添加新功能**：遵循现有模块化设计模式
...
> 💡 **提示**: 首次使用建议从`hello.txt`和`pg.txt`等小文件开始测试，熟悉操作后再处理大文件。
